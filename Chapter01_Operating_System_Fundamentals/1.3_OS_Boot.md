...
- 通过BIOS中断`int 0x15`获取**扩展内存大小(Extended Memory Size)**，存入`0x90002`处
...
**注意点**
- ⚠️ **实模式依赖**：setup阶段仍使用BIOS中断（`int`指令），因为此时还在**实模式(Real Mode)**下运行
...
**理论**
- `setup`模块在移动system模块后，需要设置**保护模式(Protected Mode)**下的核心数据结构
...
**理论**
- **保护模式(Protected Mode)**下，`cs:ip`的解释方式与实模式完全不同
...
**注意点**
- ⚠️ **本质差异**：保护模式下段寄存器不再是地址的一部分，而是**选择子(Selector)**，用于查表
...
call  empty_8042
mov   al,#0xD1
out   #0x64,al        ; 发送命令到8042：写P2端口
call  empty_8042
mov   al,#0xDF
out   #0x60,al        ; 数据0xDF：选通A20地址线
call  empty_8042

; 初始化8259（中断控制器）的机械化代码...

mov   ax,#0x0001
mov   cr0,ax          ; 设置cr0寄存器：PE位=1，启动保护模式
jmpi  0,8             ; 长跳转，cs=8，ip=0

empty_8042:
    .word  0x00eb,0x00eb  ; 短跳转延迟
    in    al,#0x64
    test  al,#2
    jnz   empty_8042      ; 等待8042缓冲区空
    ret
```

**cr0寄存器结构**
```
位0 (PE): Protection Enable = 1 → 启动保护模式
位31 (PG): Paging = 1 → 启动分页（后续由head.s设置）
```

**注意点**
- ⚠️ **A20历史遗留**：8086只有20位地址线（1MB），80286以上有24/32位，但兼容模式下A20被关闭，需要显式开启才能访问1MB以上内存
- 💡 **jmpi 0,8含义**：`cs=8`是GDT选择子（指向第1个代码段描述符），`ip=0`跳转到该段基址（即0地址）
