...
- [*知识点5: 系统调用的核心机制*](#id5)
- [*知识点6: 库函数封装与 _syscall3 宏*](#id6)
- [*知识点7: int 0x80 中断门设置*](#id7)
- [*知识点8: 中断处理程序 system_call*](#id8)
- [*知识点9: sys_call_table 与系统调用分发*](#id9)
- [*知识点10: 安全实现与不安全实现对比*](#id10)
...
**理论**
- 直观想法：实现一个 `whoami()` 系统调用，字符串 `"lizhijun"` 放在操作系统内存中（系统引导时载入），用户程序直接取出来打印
- 用户程序代码（不安全实现）：
```c
whoami() {
...
```
- **安全隐患**：
  - 都在内存中，用户程序可以**随意访问任何数据**
  - 可以看到 root 密码，可以修改它
  - 可以通过显存看到别人 word 里的内容
  - 不能随意调用数据，不能随意 `jmp`

**注意点**
- ⚠️ **关键警告**：直接让用户程序访问操作系统内存是**极其危险的**——内存隔离是操作系统的基本安全底线
- 💡 **理解技巧**："内存都是我买的"这个想法直观但错误——操作系统必须隔离不同程序的内存空间
...
**理论**
- **内核态(Kernel Mode)** 可以访问任何数据，**用户态(User Mode)** 不能访问内核数据
- 对于指令跳转也一样受限——用户态不能随意跳转到内核代码
- **实现了隔离**
- **内核(用户)段**：将内核程序和用户程序隔离！！！
...
**注意点**
- ⚠️ **关键区分**：隔离不是软件约定，而是**硬件机制**——处理器硬件层面区分特权级
- 💡 **理解技巧**：保护环像同心圆，越内层权限越高，外层无法直接访问内层资源
...
- **CPL(Current Privilege Level)**：当前程序执行在什么态（哪层环）？
  ...
  - `0` 是内核态，`3` 是用户态
- **RPL(Requestor Privilege Level)**：请求特权级，访问的数据段 DS 的最低两位
- **DPL(Descriptor Privilege Level)**：描述符特权级，目标段/门的特权级
- **特权级检查规则**：
  - `DPL ≥ CPL`（目标段的特权级必须不低于当前特权级）
  - `DPL ≥ RPL`（目标段的特权级必须不低于请求特权级）

**注意点**
- ⚠️ **关键区分**：CPL 看 CS（当前执行段），RPL 看 DS（请求访问的数据段），DPL 看目标段的描述符
- 💡 **理解技巧**：数字越小特权越高，所以检查是 **DPL ≥ CPL**（低特权不能访问高特权）
- 🔄 **知识关联**：这是 x86 段保护机制的核心，是用户态无法访问内核内存的硬件基础
- 📋 **术语提醒**：CPL`当前特权级`、RPL`请求特权级`、DPL`描述符特权级`
...
**理论**
- **硬件提供了"主动进入内核的方法"**
- 对于 Intel x86，那就是**中断指令 `int`**
- `int` 指令将使 CS 中的 CPL 改成 `0`，**"进入内核"**
- 这是**用户程序发起的调用内核代码的唯一方式**
- 此时，`CPL=3` 而 `DPL=0`，正常情况下无法访问，但通过中断门可以合法切换

**注意点**
- ⚠️ **关键区分**：`int` 指令是用户态主动进入内核态的**唯一合法途径**——不是 jmp，不是 call
- 💡 **理解技巧**：中断门就像"有守卫的大门"——用户不能直接翻墙，但可以走大门按流程进入
...
- 💡 **理解技巧**：库函数就像"翻译官"——把熟悉的 `printf` 翻译成底层的 `int 0x80`
...
```
- `__NR_write` 是系统调用号，放在 `eax` 中（如 `__NR_write = 4`）
- `eax` 也存放返回值
...
- 💡 **理解技巧**：`__NR_##name` 是宏拼接——`__NR_write` 展开为系统调用号 `4`
