...
- 💡 **理解技巧**：像电源插座一样，用户只需要知道如何插拔，不需要了解电网如何运作
- 📋 **术语提醒**：接口`Interface`——连接两个系统并转换信号的边界
...
- 操作系统直接面对用户吗？**不是**
- 用户使用计算机的链路：
  - 用户 → `应用软件(Application Software)` → `操作系统(Operating System)` → `计算机硬件(Computer Hardware)`
- **操作系统接口**连接的不是用户，而是**上层用户和操作系统软件**
- 用户通过命令（如键入 `hello`）与计算机交互，最终变成屏幕上的像素
- 操作系统接口的本质：**方便了使用，屏蔽了细节**

**注意点**
- ⚠️ **关键区分**：操作系统接口**不直接连接用户**，而是连接应用软件和操作系统
- 🔄 **知识关联**：这是分层设计思想的体现——操作系统位于硬件和应用软件之间
