...
**那么`exec`又是如何开始执行的呢**
- **`exec` 的执行入口**：
  1. 进入 `exec()` 之前，子父线程的用户态执行的相同的代码，但是接下来子线程需要开始执行不同的代码
  2. 用户输入 `hello` 命令，`exec(hello)` 加载并执行 `hello` 程序
     - hello是一个可执行程序，通过编译连接生成，在磁盘中有
     - 将可执行文件读入到内存中，从其可执行文件文件头拿出**入口地址`a_entry`**(后面要用)
  3. `exec` 是系统调用，同样通过 `INT 0x80` 进入内核
- **`exec` 系统调用内核入口**：
  ```asm
  _system_call:
      push %ds .. %fs
      ...
      call sys_execve
  ```
  1. 之前我们知道线程执行完内核代码，可以通过 `iret` 跳回到用户态
  2. 但是在exec里面跳回来后，我们还需要执行可执行文件 -- 如何控制这个执行地址呢？
  3. 关键就是要找到条回来执行的 `pc`，也就是 `EIP`

- **`sys_execve` 调用 `do_execve`**：
  ```asm
  _sys_execve:
      lea EIP(%esp), %eax       ; 计算 EIP 偏移（EIP寄存器地址 = 0x1C）
      pushl %eax
      call _do_execve
  ```
  1. 正因为上述原因，**我们需要将 `EIP` 指向`a_entry`**
  3. 它把内核栈里保存的 `EIP` 位置（也就是中断压栈时的返回地址）传给 `do_execve`。
- **栈偏移量计算**：
  ![alt text](images/44.png)
  >⚠️ `EIP = 0x1c` 表示的是偏移量，也就是离 `esp` 有多远，并非 `EIP` 保存的值
  >⚠️ 通过 `EIP(%esp) = esp + 0x1C` 可知 `ret` 刚好指向的就是 `EIP`，两寄存器位置都在28，所以通过修改 `EIP` 来调整执行位置 
- **`do_execve` 的核心操作**：
  ```c
  int do_execve(* eip, ...) {
  ...
  ```
  1. `do_execve` 拿到这个指针，直接修改栈里保存的 `EIP`，把它改成新程序 `ex.a_entry` 的入口地址。同时把用户栈指针（SP）也换成新程序的。
  2. `sys_execve` 返回后走 `iret`，CPU 从栈里弹出的 `EIP` 已经不是原来的返回地址了，而是新程序的入口。于是 CPU 直接跑去执行新程序了。

- **`eip` 指针计算**：
  - `eip[0] = esp + 0x1C` — 用户态返回地址（PC）
  - `eip[3] = esp + 0x28` — 正好是 SP（用户栈指针）
  - **为什么要修改SP?**： 
    - 我们要的是`fork`出一个**运行cmd的线程**。
    - 修改内核栈上保存的 SP，是为了让 `iret` 回到用户态时，CPU 直接切到新程序`cmd`的用户栈顶开始执行，而不是继承旧程序那堆废弃的栈数据。
