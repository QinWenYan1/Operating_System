# 📘 L8 CPU 管理的直观想法 (Intuitive Ideas of CPU Management)
...
**注意点**
- ⚠️ **关键区分**：这种"设 PC 初值"的方式只能让 CPU 顺序执行一个程序，无法处理多个程序
- 🔄 **知识关联**：L2 揭开钢琴的盖子 — CPU 开机时 `CS=0xFFFF, IP=0x0000` 就是设置 PC 初值的过程
- 📋 **术语提醒**：`单道程序(Single Program)` — 一次只加载一个程序到内存执行
...
- 示例程序（含 I/O 操作）：
  ...
  ```
- 问题分析：
  - `fprintf` 用一条其他计算语句代替：假设执行时间为 $0.015 \times 10^7$ 单位
  - 实际的 `fprintf`（磁盘 I/O）执行时间：$0.859 \times 10^3$ 单位
...
- 💡 **理解技巧**：CPU 像"火箭"，I/O 像"蜗牛"——如果让火箭等蜗牛，火箭大部分时间都在发呆
- 🔄 **知识关联**：L1 什么是操作系统 — OS 的核心价值之一就是"高效使用硬件"，避免 CPU 空转
- 📋 **术语提醒**：`I/O(Input/Output)` — 输入/输出操作，如磁盘读写、键盘输入、显示输出
...
**理论**
- 解决方案：**多道程序(Multi-programming)** — 多个程序同时在内存中，交替执行
- 单道程序 vs 多道程序对比：

| 场景 | 单道程序 | 多道程序 |
|:-----|:---------|:---------|
| 程序 A | CPU(10) → DEV1(15) → CPU(20) → DEV2(30) | CPU(10) → DEV1(15) → CPU(20) |
| 程序 B | — | DEV2(25) → CPU(35) → DEV1(45) |
| 总时间 | 80 | 45 |
...
- 💡 **理解技巧**：就像一个人同时处理两个任务——等水烧开的时候去切菜，而不是干等着
- 📋 **术语提醒**：`多道程序(Multi-programming)` — 内存中同时存放多个程序，共享 CPU 时间
...
- 💡 **理解技巧**：并发就像"快速切换频道的电视"——看起来多个节目同时在播，实际上是快速切换
- 🔄 **知识关联**：L4 操作系统接口 — 用户通过 `fork()` 创建多个进程，OS 负责并发调度
- 📋 **术语提醒**：`并发(Concurrency)` — 逻辑上同时推进；`并行(Parallelism)` — 物理上同时执行
...
- 💡 **理解技巧**：就像打游戏存档——存档不能只存"当前关卡"，还要存血量、装备、经验值等所有状态
- 🔄 **知识关联**：L3 操作系统启动 — 内核初始化时也涉及寄存器的保存和恢复（中断上下文）
...
- 💡 **理解技巧**：程序像"菜谱"，进程像"正在炒菜的过程"——菜谱是死的，炒菜是活的，需要记录火候、时间、进度等
- 🔄 **知识关联**：L7 回顾和计划 — CPU 管理的核心抽象就是"进程"，通过 `fork()` 创建
...
- **关键机制**：CPU 取指执行 → I/O 等待暴露空转 → 多道程序并发 → 切换需保存上下文 → PCB 记录进程状态
- **易混淆概念对比**：
...
| 程序 | 静态代码，存在磁盘上 | 死的 |
| 进程 | 运行中的程序，有状态 | 活的 |
| 并发 | 一个 CPU 上交替执行 | 逻辑同时 |
| 并行 | 多个 CPU 上同时执行 | 物理同时 |

- **常见考试陷阱**：
  - ❌ 认为 "并发 = 并行" → ✅ 并发是交替执行，并行是同时执行
  - ❌ 认为 "进程就是程序" → ✅ 程序是静态代码，进程是动态执行实例（包含 PCB）
  - ❌ 认为 "切换只需要保存 PC" → ✅ 需要保存所有寄存器（完整上下文）

**记忆口诀**：
> "CPU 取指执行忙，I/O 等待空转伤，多道程序交替上，利用率翻倍涨；切换不能只存 PC，ax bx 都要装，程序运行不一样，PCB 里面全收藏！"
