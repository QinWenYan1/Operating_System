...
- 💡 **理解技巧**：把 `P/V` 想象成"门票系统"——`P` 是取票进门，`V` 是离场还票
- 🔄 **知识关联**：与 L16-L17 的信号量理论定义衔接，这里是工程实现层
...
- 📋 **术语提醒**：`task_struct` 是 Linux 进程控制块 `PCB(Process Control Block)`
...
- 🔄 **知识关联**：与文件系统的 `open()` 返回 `fd` 的模式完全一致
- 📋 **术语提醒**：`sd` = semaphore descriptor，信号量描述符
