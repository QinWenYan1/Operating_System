...
- `sys_write` 在 `fs/read_write.c` 中实现：
  ...
  ```
- **主要任务**：
  - 用户进程调用 `write` 时，内核通过 `fd` 查本进程**私有打开文件**表拿到 `inode`，若是普通文件则转交 `file_write` 处理
- **核心机制**：`open` 建立 "PCB → 访问打开文件表 → 找到目标inode → 读写数据盘块" 这条链
...
- **第一条路：读写磁盘** 
  - 文件名 → 目录解析找到 **inode** → 逻辑块号通过 `_bmap` 映射为**物理盘块号** → `make_request` 构造请求并放入电梯队列 → `do_hd_request` 将扇区号转为 **CHS** → `hd_out` 通过 `out` 指令向磁盘控制器发命令 → 控制器驱动磁头寻道、读写。
- **第二条路：输出到显示器**

  - `write(fd)` → 通过 `filp[fd]` 找到 **inode** → 判断为字符设备 → 经 `crw_table` 分发到 `tty_write` → 字符放入 `write_q` → `con_write` 从队列取字符 → **`mov` 写入显存**（`pos` 指向 0xB8000）→ **同时用 `out` 指令更新 VGA 控制器寄存器**（光标位置、屏幕滚动等）→ 显卡扫描显存并显示到屏幕。

**说明**：`mov` 负责把字符"画"到显存里，`out` 负责告诉显卡控制器"光标现在在哪""要不要滚屏"——两者配合完成最终显示。
- 统一的视图：
...
- **`proc` 读的实现**：
  ...
  - `/proc/psinfo` 是 proc 文件系统中的**虚拟文件**，其 inode 为 PROC 设备，回调函数是 `proc_read`，直接向用户空间输出进程信息
  - `/proc/psinfo` 是 `procfs` 的虚拟文件，**是内核暴露进程数据的软件接口**，不是硬件设备
  >⚠️ **注意**：它不是空壳 `flag`，而是内核数据的"窗口"——窗框（inode）上贴着操作说明（函数指针），透过窗户才能看到内核实时生成的数据
  ```c
...
- 文件创建：
  ...
  ```
- **主要任务**：系统初始化时创建 `/proc` 目录，并注册一个类型为 `S_IFPROC`、权限只读的虚拟文件节点 `/proc/psinfo`。
