...
- `open` 的核心作用是**读入 inode**

**注意点**
- ⚠️ **关键区分**：`S_ISREG` 判断是否为普通文件，只有普通文件才走 `file_write`
- 💡 **理解技巧**：把文件当成一扇门——`open` 把门打开并记住门牌号（inode），`write` 通过门牌号找到背后的数据
- 🔄 **知识关联**：L2 系统调用 → L6 文件系统 → 本节 file_write 实现
- 📋 **术语提醒**：`fd`（文件描述符）`file descriptor`，`inode`（索引节点）`index node`
