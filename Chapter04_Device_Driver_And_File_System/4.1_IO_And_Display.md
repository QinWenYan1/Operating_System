...
**既然知道了 `inode`，那么 `inode` ，`file` ，`filp` 从哪里来呢**
- **核心结论**：因为是被 `current` 指向，所以是从 **`fork`** 中来
- **进程拷贝机制：`copy_process`**
    1. **复制 PCB**：把当前进程（父进程）的完整进程控制块（`current`）拷贝给新进程（`p`），让子进程继承父进程的寄存器、内存空间等执行上下文。
    2. **共享已打开文件**：遍历子进程的文件描述符表（`filp[]`），对每个已打开的文件对象引用计数（`f_count`）加 1，使父子进程**共享同一套文件描述符和文件偏移**，而非复制底层文件。
    ```c
    int copy_process(...) {
        *p = *current;  // 拷贝整个 PCB
        for (i=0; i<NR_OPEN; i++)
    ...
    ```
    - `filp` 就是内核里代表"一个已打开文件"的指针，`fork` 时父子通过共享同一个 `filp` 来实现文件描述符的继承
...
- 标准文件描述符的初始化链：
    ...
    ```
- **主要任务**：
    1. **初始化标准 IO**：打开当前虚拟终端 `tty0`（`fd=0`），并通过 `dup(0)` 复制出 `fd=1`（标准输出）和 `fd=2`（标准错误），让 `stdin/stdout/stderr` 都指向同一个终端。

    2. **启动用户态 Shell**：`execve("/bin/sh")` 用 Shell 程序替换当前 init 进程，系统从此进入交互式命令行环境。
    - `tty0` 是 Linux 里的当前**虚拟控制台**（current virtual console）
        - `tty0` 就是"当前亮着的那个黑屏终端"

- 所以回到最开始的问题：`filp[1]` 其实就是终端设备
