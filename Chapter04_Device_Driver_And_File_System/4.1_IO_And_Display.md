...
- 文件位置：`linux/fs/open.c`
    ...
    ```
- **主要任务**：
    1. **"打开会话"**：`f` 是从内核全局打开文件表 `file_table` 中分配出来的一个空闲 `struct file` 指针。
    2. **解析文件路径**：`open_namei` 根据文件名读入 `inode`（文件元数据）。

    3. **分配 fd 并建立映射**：根据文件名找到文件，在进程里登记一下并分配一个编号（fd），以后用户就拿这个编号来读写文件。
...
- 做以上这些都是在干什么？：`Open` 将目标文件 `inode` 读入， 之后的 `write` 对这个 `inode` 进行操作
    ...
    > ⚠️ **注意**：`filp`[] 管"进程里用哪个编号找文件"，`file_table` 管"系统里所有打开会话的实体"——分开才能既隔离 `fd` 命名空间，又支持共享和多次打开。

- **为什么 `filp[]` 中放的是 struct file 的指针而不是 inode?**:
    - 因为 **inode 是文件的"身份证"**（全局共享，存权限、大小、数据块位置），而 **`struct file` 是"打开会话"**（存读写位置 `f_pos`、打开标志 `f_flags` 等）。

    - 同一个文件被不同进程打开，或同一个进程打开两次，**inode 只有一份**，但每个打开实例都需要**独立的读写光标**。`filp[]` 存 `struct file *` 就是为了隔离这些会话状态——如果直接指向 inode，所有进程共享同一个光标，文件读写就乱套了。
- **已打开的文件表 `filp[]` 是什么？**
    - 打开的文件 = 内核已为它分配了 `struct file` 实例、并在进程 `filp[]` 中有 `fd` 指向它的那些文件，大多数是 `open()` 得来的，少数是继承的。
---
...
- **字符设备读写表：`crw_table`**
    ...
    ```
- **主要任务**：
    - 找到`/dev/tty0` 对应**第 4 个**条目 → `rw_ttyx` 并调用
...
- **转到 `rw_ttyx`去执行**：
    ...
    ```
- **主要任务**：
    1. 对 `tty0` 进行读写操作，由于上面 `sys_write` 调用时传入的 `rw` 参数是 `WRITE`，所以就是写
    2. 于是调用 `tty_write` 函数
...
- 文件位置：`linux/kernel/tty_io.c`
    ...
    ```
- **主要任务**：
    1. 根据 `channel` 编号在 `tty_table` 里找到对应的终端控制块（`tty_struct`），确定要往哪个终端写。
    2. 检查该终端的**写队列**（`write_q`）是否已满，满了就让当前进程睡眠等待，腾出空间后再继续写入数据
    3. 进来的数据都是在缓冲区，需要写入直接去缓冲区取
...
- **继续 `tty_write` 剩下的逻辑**
    ...
    ```
- **主要任务**：
    1. 从用户缓冲区逐个字符读取，做回车转换/大小写转换等处理后，塞入终端写队列。
    2. 写队列塞满或数据写完后，调用底层 `tty->write` 驱动函数把队列数据真正刷到硬件。
...
- **`tty_table` 初始化**
    ...
    ```
- **主要任务**：
    1. 开机时内核在 `tty_table[0]` 这个位置登记了第一个终端——控制台（`tty0`），告诉它"你的输出函数是 `con_write`，读写队列现在都是空的"
    > 📋 **术语提醒**：`con_write` = Console Write，控制台写函数，直接操作显存
    > ⚠️注意：这就是设备驱动的本质，把"操作硬件的函数"封装好，注册到内核的接口表里，让上层能通过统一方式调用。
...
- 文件位置：`linux/kernel/chr_drv/console.c`
    ...
    ```
- **主要任务**：
    1. 从终端写队列取字符，若是可打印字符（ASCII 32-126），通过内联汇编把**字符 + 显示属性** `_attr`写入 VGA 显存（`pos` 指向的内存地址）。
    2. 每写一个字幕，光标位置前进 **2 字节**（显存里 1 字节存字符、1 字节存颜色属性）。
...
- `0x90000` 和 `0x90001`：这是 `setup.s` 从 `BIOS` 读取的初始光标位置（2 字节数据），临时存在常规内存末尾（`0x90000` 处），是"光标该在哪闪烁"的配置参数。
- 屏幕上的每个格子都在显存里的一个固定"座位"，pos 就是座位号，CPU 往座位里填字符，显卡自动把座位表投影到屏幕上
    - **`con_write` 里的 `pos`**：内核维护的**显存写入指针**，CPU 往这个地址写字符，直接刷到屏幕。
    - **0x90000 处的光标位置**：BIOS 保存的**行列坐标**（第几行第几列），内核启动时读取它，**换算成显存偏移后赋值给 `pos`**。
...
**还有一个值得注意的是...**
- **显存是什么**：
    - 显存（显示内存）是显卡用来暂存屏幕图像数据的专用内存区域 -- 数据缓冲区（像一块专门存画面的内存）
    - **CPU 往这里写字幕和颜色，显卡从这里读取并刷新到显示器**
- **`0xA0000–0xC0000` 显存里放的什么**：`0xA0000–0xBFFFF` 是 VGA 显存（其中 `0xA0000–0xAFFFF` 是图形模式显存，`0xB0000–0xBFFFF` 是文本模式显存）
- **CGA 彩色图形适配器**(`Color Graphics Adapter`)标准
  - 显存地址范围：`0xb8000` ~ `0xbc000`
- `movw %%ax, %1` 一次写入 2 字节——低字节是字符，高字节是属性
...
- **主要流程**：
    1. 用户程序调用 `write()` → 进入内核 `sys_write`（`read_write.c`），通过当前进程 `filp[fd]` 找到 `struct file` 和 inode。

    2. `sys_write` 判断 inode 为字符设备（`S_ISCHR`）→ 调用 `rw_char`（`char_dev.c`）。

    3. `rw_char` 查 `crw_table`（字符设备函数跳转表，以主设备号为索引）→ 找到该设备对应的处理函数 → 分发到 `tty_write`。

    4. `tty_write`（`tty_io.c`）从用户缓冲区（`fs` 段）逐个取字符，做回车转换、大小写转换后，塞入该终端的 `write_q`。

    5. 队列写完后，`tty_write` 调用 `tty_struct` 里的 `write` 函数指针，即 `con_write`（`console.c`）。

    6. `con_write` 从 `write_q` 取字符，通过内联汇编把**字符 + 属性**写入 VGA 显存（`pos` 指向的地址），`pos += 2`，字符最终显示在屏幕上。
