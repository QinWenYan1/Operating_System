...
- **主要流程**：
    1. 用户程序写 stdout（fd=1）→ 触发 `sys_write`，内核通过当前进程的 `filp[1]` 找到打开的文件对象（`struct file`），进而拿到 inode。

    2. 判断 inode 是字符设备（`S_ISCHR`）→ 走 `rw_char` → 进入 `tty_write`，根据设备号在 `tty_table` 里定位到具体的 `tty_struct`（终端控制块）。

    3. `tty_write` 逐个从用户缓冲区取字符，做完回车转换、大小写转换等处理后，塞入该终端的 `write_q`（写队列）。

    4. 队列满或数据写完后，调用 `tty_struct` 里的 `write` 函数指针，即 `con_write`。

    5. `con_write` 从 `write_q` 取字符，若是可打印字符（ASCII 32-126），通过内联汇编把**字符 + 显示属性**写入 VGA 显存地址。

    6. 每写一个字符，显存指针 `pos` 前进 **2 字节**（1 字节字符 + 1 字节属性），光标后移，字符最终显示在屏幕上。
