...
- 计算机硬件架构包含：`PCI总线`(`PCI Bus`)、`图形控制器`(`Graphics Controller`)、`IDE控制器`(`IDE Controller`)、`总线控制器`(`Bus Controller`)、`CPU-内存总线`(`CPU-Memory Bus`)、`扩展总线`(`Expansion Bus`)

**注意点**
- ⚠️ **关键区分**：设备管理是操作系统四大资源管理之一，用户通过**设备文件**(`Device File`)而非直接操作硬件寄存器来访问外设
- 🔄 **知识关联**：进程 (`fork()`) 带动整个系统的视图，所有资源管理都围绕进程展开
...
| 步骤 | 操作 | 说明 |
|-----|------|------|
| 1 | 向控制器中的寄存器读写数据 | CPU 发起操作 |
| 2 | 发出写命令 | CPU 向控制器下达指令 |
| 3 | 控制器完成真正的工作 | 控制器独立执行，不占用 CPU |
| 4 | 读数据到内存 | 数据搬运（可由 DMA 完成）|
| 5 | 向 CPU 发出中断 | 异步通知操作完成 |

- **关键硬件**：显卡(`Graphics Card`)、键盘(`Keyboard`)
- **核心要点**：控制器(`Controller`)完成真正的工作，并向 CPU 发中断信号；CPU 只负责向控制器中的寄存器读写数据

**注意点**
- ⚠️ **关键区分**：控制器独立执行工作，CPU **不需要轮询等待**，通过中断(`Interrupt`)异步获知完成
- 💡 **理解技巧**：CPU 是"指挥官"，控制器是"执行者"，中断是"报告员"
- 🔄 **知识关联**：这与 CPU 调度、进程同步中的信号量机制类似——都是"发起-执行-通知"的三段式结构
...
| 问题 | 答案 |
|-----|------|
| 显然是拷贝来的，那么是谁一开始打开的？ | `init()` 进程打开 `/dev/tty0` |
| shell 进程启动了 whoami 命令，shell 是其父进程 | 说明 fd=1 通过 **fork 继承链** 传递 |

**注意点**
- ⚠️ **关键区分**：`dup(0)` 是复制文件描述符，不是复制文件内容——两个 fd 指向同一个 `file` 结构
- 💡 **理解技巧**：fork 时子进程继承父进程的 `filp[]` 数组，引用计数 `f_count++` 防止父进程关闭后子进程无法使用
- 🔄 **知识关联**：这与之前学过的 `fork()` 进程创建、`copy_process` 完全对应——"进程带动整个系统的视图"
...
**注意点**
- ⚠️ **关键区分**：`open_namei()` 负责路径解析，最终找到的是 `inode`——其中包含设备类型和设备号
- 🔄 **知识关联**：文件系统的目录解析机制在这里被复用，设备文件也是文件系统的一部分
- 📋 **术语提醒**：`file_table[]` 是系统级文件表，`filp[]` 是进程级文件指针数组
...
- `/dev/tty0` 的 `inode` 中的信息是**字符设备**(`Character Device`)，`S_ISCHR` 判断为真
- `inode->i_zone[0]` 存储**设备号**(`Device Number`)
...
- 🔄 **知识关联**：设备号分为主设备号(`Major Number`，标识设备类型)和次设备号(`Minor Number`，标识具体设备实例)
