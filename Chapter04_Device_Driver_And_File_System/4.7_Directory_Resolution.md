...
- 用户读取文件 `test.c` 第 202-212 字节的完整流程，串联了文件系统与磁盘管理的全部层次：
  ...
  6. **磁盘中断**完成实际读取
- `open(/xx/test.c)` 的目录解析过程：
  - 从根目录 `/` 开始，读入 `/` 的内容找到 `xx`
  - 再从 `xx` 目录中找到 `test.c` 的 `inode`
- 数据结构关系：`inode` 数组 → 数据盘块（12-23 等盘块号）
...
用户 read(fd)
    ...
    → 找到 FCB 和 file
    → 找盘块
    → add_request(789) 写入电梯队列
    → 从队列取出 789 → 算出 cyl, head, sector
    → outp(cyl, head, sector) 写磁盘控制器
    → 磁盘中断
```

**注意点**
- ⚠️ **关键区分**：目录解析是文件系统的第一步，没有它就无法找到文件的 `FCB` 和盘块位置
- 💡 **理解技巧**：把目录解析想象成"按图索骥"——从根目录出发，一层一层目录名找下去，直到定位到目标文件
- 🔄 **知识关联**：L31 讲了目录与文件系统的概念，L32 是实现代码；L28 讲了生磁盘的使用，目录解析之后才会调用磁盘驱动
- 📋 **术语提醒**：`FCB(File Control Block)` = 文件控制块，包含文件元信息和盘块位置
...
- 🔄 **知识关联**：L30 讲了文件使用磁盘的实现，L31 讲了目录结构，L32 是从系统调用入口看代码实现
- 📋 **术语提醒**：`inode(Index Node)` = 索引节点，存储文件的元信息（不包括文件名）
...
- 四句话对应目录树的四个重点：
  ...
  2. **读目录项**：`find_entry(&inode, thisname, namelen, &de)` 从当前目录中读取一个目录项
...
- 💡 **理解技巧**：把 `get_dir` 想象成"走到目标文件所在的目录"，而不是"找到文件本身"
- 🔄 **知识关联**：L31 目录树的结构（/ → var → my → test.c）就是 `get_dir` 的解析对象
- 📋 **术语提醒**：`thisname` = 当前要解析的路径分量；`namelen` = 路径分量长度
...
- 🔄 **知识关联**：L9 讲过 `task_struct` 和进程切换，这里再次看到 `current` 指针的作用
...
- 步骤：
  ...
  2. 设置 `i_dev = dev`，`i_num = nr`
  3. 调用 `read_inode(inode)` 从磁盘读取实际 inode 数据
- `read_inode()` 的核心是计算 inode 所在的**磁盘块号**：
  - 磁盘布局：引导块 → 超级块 → i节点位图 → 盘块位图 → **i节点区** → 数据区
  - 从 block=2（超级块后）开始，跳过 i节点位图和盘块位图占据的块，定位到 i节点区
  - 公式：`block = 2 + s_imap_blocks + s_zmap_blocks + (inode->i_num - 1) / INODES_PER_BLOCK`
  - 在块内偏移：`(inode->i_num - 1) % INODES_PER_BLOCK`
...
- 💡 **理解技巧**：inode 在磁盘上是连续存储的数组，计算块号就像"数组下标 → 所在页"的地址转换
- 🔄 **知识关联**：L22 多级页表和快表、L30 文件使用磁盘的实现，都涉及"逻辑地址 → 物理地址"的转换思想
- 📋 **术语提醒**：`s_imap_blocks` = 超级块中 i节点位图占用的块数；`s_zmap_blocks` = 盘块位图占用的块数；`INODES_PER_BLOCK` = 每块容纳的 inode 数量
