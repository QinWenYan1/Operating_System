...
- **代码**：
    ...
    ```
-   **主要任务**：
    - **`trap_init`**：内核启动时初始化整个中断/异常处理框架，填充 IDT 使 CPU 遇到异常或系统调用时能跳转到正确的内核处理函数。
    - **`set_trap_gate`**：向 IDT 的指定槽位写入一个陷阱门描述符，将某个中断向量号与对应处理函数的入口地址及特权级绑定。
...
- **代码**: 
    ...
    ```
- 主要任务：
    - `address &= 0xfffff000;`：`&0xfffff000` 是取出页目录项中的**页表地址**，因为保留了高20位，就是虚拟页号
    - `bread_page`：到磁盘读入文件 
...
- **主要任务**：
    ...
    - `page_table[(address >> 12) & 0x3ff] = page | 7;`：**建立映射关系**
        - `page \| 7`  设置页表项属性：P=1（存在）、R/W=1（可读写）、U/S=1（用户态） 
        - `(address >> 12) & 0x3ff`  **提取页表索引**（中间10位）

- **核心区别：一个是“物理地址”，一个是“数组下标”。**

    - **页表基址（或页表项）的前20位**：存的是**物理页框号**。它指向一张实际的物理页（比如页目录或页表在内存中的起始位置），拼接上12个0就是真实的物理地址。

    - **虚拟地址中间的10位**：是**页表索引**。它相当于数组下标，告诉 CPU “在我刚才找到的那张页表里面，我要第几个条目”。

- **一句话总结**：前20位是“某张表在物理内存的哪一页”，中间10位是“我要那张表里的第几项”。 
