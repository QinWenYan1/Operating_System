...
截至目前，仓库实际仅完成了**第 1 章（操作系统基础）**和**第 2 章（进程与线程）**的部分小节，第 3、4 章尚未创建。
...
└── Chapter02_Process_Thread/               # 第2章：进程与线程
    ...
    └── images/                             # 第2章配图（18 张 PNG）
```

注意：根目录 `README.md` 的章节导航中，第 2 章链接写作 `./Chapter02_Processes_and_Threads/README.md`，但实际目录名为 `Chapter02_Process_Thread/`，目前该链接无法直接跳转。
...
- 一个 `README.md`（第 1 章已存在；第 2 章目前未创建）作为该章导航页。
...
3. **链接准确性**：创建新章节或新小节后，应同步更新根目录 `README.md` 和对应章节目录页的导航链接，注意目录名大小写和单复数（例如 `Chapter02_Process_Thread`）。
